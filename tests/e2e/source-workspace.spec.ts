...
test('provider-only routes, PDF upload, comparison, private preparation and source changes',async({page,browser})=>{
 ...
 try{
  ...
  await page.goto('/tenders/th-6732969');await page.getByRole('button',{name:'Analyze documents',exact:true}).click();await expect(page.getByRole('button',{name:'Read document',exact:true})).toHaveCount(0);
  await page.getByText('Upload a PDF',{exact:true}).click();await page.getByLabel('Attach tender PDF for AI').setInputFiles({name:'supporting-notice.pdf',mimeType:'application/pdf',buffer:pdfBytes()});await expect(page.getByLabel('Document to analyze')).toContainText('supporting-notice.pdf');
  ...
  await page.getByLabel('Complete task 1').check();await page.getByLabel('Internal deadline').fill('2026-09-20');await page.getByRole('button',{name:'Save changes'}).click();await expect(page.getByText('Bid preparation saved',{exact:true})).toBeVisible();
