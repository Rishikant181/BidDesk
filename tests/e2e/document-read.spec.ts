...
test("reads a linked official PDF through the browser",async({page})=>{
 ...
 const workspace=await(await page.request.get("/api/data")).json();const tender=workspace.tenders.find((t:{reference:string})=>t.reference.includes("PT-18/26-27"));expect(tender).toBeTruthy();
