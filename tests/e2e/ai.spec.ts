...
test("document draft, private eligibility, matching, caching and isolation",async({page,browser})=>{
 ...
 const workspace=await (await page.request.get("/api/data")).json();const tender=workspace.tenders.find((t:{category:string;closesAt:string})=>t.category!=="Unclassified"&&t.closesAt>new Date().toISOString());expect(tender).toBeTruthy();
 ...
 expect((await page.request.post("/api/ai",{data:{action:"extract",documentId:document.id,pages:[1],chunkIndex:0}})).status()).toBe(400);
 await page.goto(`/tenders/${tender.id}`);await page.getByRole("button",{name:"Analyze documents",exact:true}).click();
 await page.getByLabel("Document to analyze").selectOption(document.id);await page.getByRole("checkbox",{name:/selected document text/}).check();await page.getByRole("button",{name:"Analyze with Gemini",exact:true}).click();await expect(page.getByRole("heading",{name:"Review AI suggestions"})).toBeVisible();
 await page.getByRole("checkbox",{name:/description/}).check();await page.getByRole("checkbox",{name:"Include requirement 1"}).check();await page.getByRole("checkbox",{name:/I checked this requirement/}).check();await page.getByRole("checkbox",{name:/I reviewed the selected suggestions/}).check();await page.getByRole("button",{name:"Save selected private findings"}).click();await expect(page.getByText("Source-backed private findings saved",{exact:true})).toBeVisible();
 const state=await (await page.request.get(`/api/ai?tenderId=${tender.id}`)).json();expect(state.findings.fields).toHaveLength(1);
 const repeat=await ai(page.request,{action:"extract",documentId:document.id,pages:[1],chunkIndex:0,consent:true});expect(repeat.cached).toBe(true);
 ...
 await page.getByRole("tab",{name:"eligibility",exact:true}).click();await page.getByRole("checkbox",{name:/selected document text/}).check();await page.getByRole("button",{name:"Review eligibility with AI"}).click();await expect(page.getByText("Test fixture evidence comparison")).toBeVisible();
 ...
 await ai(page.request,{action:"profile",tenderId:tender.id});await page.goto("/discover?recommended=true");await page.getByRole("checkbox",{name:/selected document text/}).check();await page.getByRole("button",{name:"Find semantic matches"}).click();await expect(page.getByText("Recommendations ready. Relevance and eligibility are assessed separately.")).toBeVisible();await expect(page.locator(".ai-match-card").first()).toBeVisible();
...
test("atomic duplicate suppression and application allowance",async({request})=>{
 ...
 const payload={action:"extract",documentId:document.id,pages:[1],chunkIndex:0,consent:true};
