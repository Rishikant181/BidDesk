...
export function stubGenerate(task:string,input:unknown):unknown {
 const data=input as {document?:SourceDocument;requirements?:{id:string;citations?:unknown[];additionalEvidence?:{id:string;text:string}[]}[];evidence?:{id:string}[];candidates?:{id:string;text:string}[]};
 ...
 if(task==="eligibility")return {items:(data.requirements||[]).map(r=>({requirementId:r.id,suggestion:"supporting evidence",explanation:"Test fixture evidence comparison",evidenceIds:[...(data.evidence||[]),...(r.additionalEvidence||[])].map(e=>e.id),citations:r.citations||[],nextAction:"Review original evidence"}))};
