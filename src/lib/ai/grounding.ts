...
export function evidenceFor(company:Company){return [{id:"capabilities",text:company.aiProfile},{id:"supporting-information",text:company.supportingInformation},...company.projects.filter(p=>p.shareWithAI).map(p=>({id:p.id,text:`${p.title}\n${p.scope}\nCompleted: ${p.completedAt||"unknown"}\nEvidence reference (user declared): ${p.evidence||"not supplied"}`}))].filter(x=>x.text.trim());}
export function mergeEligibility(r:Requirement,company:Company,deadline:string,suggestion:EligibilitySuggestion|undefined,docs:SourceDocument[],additionalEvidence:{id:string;text:string}[]=[]){const rule=assess(r,company,deadline);const evidenceIds=new Set([...evidenceFor(company),...additionalEvidence].map(e=>e.id));const grounded=!!suggestion&&suggestion.requirementId===r.id&&suggestion.citations.length>0&&suggestion.citations.every(c=>validCitation(c,docs)&&r.citations?.some(source=>source.documentId===c.documentId&&source.page===c.page&&normalize(source.quote).includes(normalize(c.quote))))&&(suggestion.suggestion!=="supporting evidence"||suggestion.evidenceIds.length>0)&&suggestion.evidenceIds.every(id=>evidenceIds.has(id));const ai=grounded?suggestion:null;let outcome=rule.outcome;if(r.complex||!r.confirmed||r.origin==="ai-assisted"&&!(r.citations?.length&&r.citations.every(c=>validCitation(c,docs))))outcome="needs review";if(outcome!=="not satisfied"&&supportsQualitativeRequirement(r,ai))outcome="appears satisfied";if(ai&&ai.suggestion!=="supporting evidence"&&outcome==="appears satisfied")outcome="needs review";return {requirementId:r.id,label:r.label,outcome,rule,ai,nextAction:ai?.nextAction||"Review the original condition and supporting evidence."};}
