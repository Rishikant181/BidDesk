...
export function assess(r: Requirement, c: Company, closesAt = "", now = new Date()): Assessment {
  ...
  if (r.type === "turnover") {
    if (c.turnover == null || r.threshold == null || !c.turnoverEvidence || !r.period || r.period !== c.turnoverPeriod || r.value !== "INR") return result("needs review","Supply evidenced turnover with matching financial period and INR units. Composite formulas need manual review.");
  ...
  if (r.type === "certification") {
    ...
    if (!cert || !cert.evidence || !cert.expiresAt) return result("needs review","Certification evidence and expiry date are missing.");
    ...
    return result(expiry >= at ? "appears satisfied" : "not satisfied",expiry >= at ? "Certificate is valid through the assessment deadline." : "Certificate expires before the assessment deadline.",cert.evidence);
  }
  if (r.type === "location") return result(c.regions.some(v=>v.trim().toLowerCase()===r.value.trim().toLowerCase()) ? "appears satisfied" : "needs review","Compare the declared operating region with the exact location clause; registration requirements require separate evidence.", c.regions.join(", "));
  return result("needs review","Review experience or composite conditions against the original document.",r.type === "experience" ? c.experience : "");
}
