...
export function profileHash(c:Company){return hash([profileText(c),planSearches(c)]);}
...
export async function getRun(ownerId:string,id:string){
 ...
 if(profileHash(c)!==r.profileHash)throw new AiError('Your profile changed. Start a new matching search.',409);
 return r;
}
...
export async function latestMatches(ownerId:string){
 ...
 if(!r)return null;
 const c=companySchema.parse(await db.collection('companies').findOne({ownerId})||{});
 return visibleRun(r,profileHash(c)!==r.profileHash||await tenderStateChanged(r));
}
