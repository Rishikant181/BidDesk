...
- [ ] Save contact/support details in Supporting information, reload and check that they persist separately from Company capabilities. Run eligibility and verify the supporting information is considered. Changing it must stale the assessment while leaving matching inputs unchanged.
...
- [ ] In Eligibility, add details and a readable PDF using Add missing information. Save & recheck must update only that requirement, preserve other results/assessments, and leave the company profile unchanged. Reload and verify the saved details and authenticated attachment link.
- [ ] Interrupt a requirement check or exhaust its test allowance. Inputs must remain saved for retry. Test unreadable/oversized PDF excerpts, stale concurrent edits, attachment replacement/removal and foreign-account access rejection.
- [ ] Enter bid-only financial figures or certificate expiry with proof. Verify local failures remain visible and removing proof restores an information gap. A full recheck must continue using saved bid-only evidence.

## 4. Documents, AI and OCR

- [ ] Upload the readable PDF to a source-backed tender. Verify page count/text and open the original PDF from Document options at the correct page. Reload and open it again.
- [ ] Run analysis with the real configured AI service using dummy documents. Verify private findings appear and persist after reload without approval or source excerpts. Verify extracted requirements appear in Eligibility automatically. Inspect accuracy against the dummy documents.
...
- [ ] Verify analyzed pages and human-reviewed pages are separate. Analysis requires no page approval; unreadable or unanalyzed gaps must remain visible.
...
- [ ] Use Update assessment without running AI. A favorable explanation/judgment must not hide a known numeric failure or unresolved document coverage.
...
- [ ] If hosted extension support is implemented, test a real signed-in Chrome TenderHut session, valid pairing, expired/reused codes, wrong tender and unsupported/oversized ZIPs. Local fixture transfer does not establish real session-download success.
