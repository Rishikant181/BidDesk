...
Optional attachment transfer uses the user's authenticated source session in Firefox. Credentials remain in the browser. A user may instead upload a PDF against an existing tender for private review; this does not create a new notice or overwrite public facts.
