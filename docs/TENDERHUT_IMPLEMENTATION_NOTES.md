...
Source fetches have a 12-second timeout, 4 MB response cap, at most two simultaneous requests per process and bounded backoff. Matching selection is heuristic, not exhaustive. Unspecified date timezones remain date-only in normalized fields. There is no official submission, awards feed, OCR, team assignment or notification delivery. Transferred binaries are private temporary local files; extraction text persists. No deployment or distributed concurrency guarantees are claimed.
